...
- You may also refer to our [Continuous Integration setup](https:
- Then build using `make -f Makefile.emscripten` while in the current directory.

**Running an Emscripten project**

To run on a local machine:
- `make -f Makefile.emscripten serve` will use Python3 to spawn a local webserver, you can then browse http:
- Otherwise, generally you will need a local webserver. Quoting [https:
_"Unfortunately several browsers (including Chrome, Safari, and Internet Explorer) do not support file:
- Emscripten SDK has a handy `emrun` command: `emrun web/index.html --browser firefox` which will spawn a temporary local webserver (in Firefox). See https:
