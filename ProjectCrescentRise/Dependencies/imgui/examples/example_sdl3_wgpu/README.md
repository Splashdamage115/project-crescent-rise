...
#### Building for desktop (WebGPU-Native) with WGPU:
 1. download WGPU-Native autogenerated binary modules for your platform/compiler from: https:
...
#### Building for Emscripten:
 1. Install Emscripten SDK following the instructions: https:
...
- `make serve` will use Python3 to spawn a local webserver, you can then browse http:
- Otherwise, generally you will need a local webserver:
  - Quoting [https:
_"Unfortunately several browsers (including Chrome, Safari, and Internet Explorer) do not support file:
  - Emscripten SDK has a handy `emrun` command: `emrun web/example_glfw_wgpu.html --browser firefox` which will spawn a temporary local webserver (in Firefox). See https:
